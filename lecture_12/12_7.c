...
int main(void) {
  ...
  print_tree(tree4);
  tree_height(tree4);
  printf("height: %d", tree_height(tree4));
  exit(EXIT_FAILURE);
}
...
/* Opgave */
int tree_height(tree_node *tree_ptr){
  int left = ((tree_ptr->leftp == NULL) ? (0) : (1 + tree_height(tree_ptr->leftp))),
      right = ((tree_ptr->rightp == NULL) ? (0) : (1 + tree_height(tree_ptr->rightp)));
