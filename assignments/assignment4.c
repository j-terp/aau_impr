...
int gcd_up(int i, int j) {
  ...
  count = 1;
  while (count <= j) {
    !((i % count) && (j % count)) ? divisor = count : 0;
    ++count;
  }
  return divisor;
}
...
int gcd_smart(int i, int j) {
  ...
  if (!j)
    return i; // Special case: when one is 0 gives the other number as the gcd
  else {
